...
# 환경 설정
source /home/rex/workspace/nl2sql/tr/test-dft/bin/activate

# 로그 디렉토리
mkdir -p ./logs

# Config 파일 (DeepSpeed 없는 버전)
CONFIG_FILE="config/unsloth_test.json"

# GPU 개수 계산
NUM_GPUS=$(echo "${GPU_IDS}" | tr ',' '\n' | wc -l)

LOG_FILE="./logs/test_GPU_${GPU_IDS}_$(date +%Y%m%d_%H%M%S).log"
